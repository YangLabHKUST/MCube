...
class DenseLayer(nn.Module):
    ...
    def __init__(self,
        ...
        if zero_init:
            ...
            nn.init.uniform_(self.linear.weight.data, -np.sqrt(6 /
                             (c_in + c_out)), np.sqrt(6 / (c_in + c_out)))
...
class GATSingleHead(nn.Module):
    ...
    def __init__(self,
        ...
        nn.init.uniform_(self.linear.weight.data, -np.sqrt(6 /
                         (c_in + c_out)), np.sqrt(6 / (c_in + c_out)))
        nn.init.zeros_(self.linear.bias.data)
        nn.init.uniform_(self.v0.data, -np.sqrt(6 / (c_out + 1)),
                         np.sqrt(6 / (c_out + 1)))
        nn.init.uniform_(self.v1.data, -np.sqrt(6 / (c_out + 1)),
                         np.sqrt(6 / (c_out + 1)))
...
class DeconvNet(nn.Module):
    ...
    def __init__(self,
                 ...
                 coef_fe,
                 sparse,  # use sparsemax or not in deconvolution layer
                 ):
        ...
        self.encoder_layer1 = GATMultiHead(
            hidden_dims[0], hidden_dims[1], n_heads=n_heads, concat_heads=True)
        self.encoder_layer2 = DenseLayer(hidden_dims[1], hidden_dims[2])
        # decoder layers
        self.decoder_layer1 = GATMultiHead(
            hidden_dims[2] + slice_emb_dim, hidden_dims[1], n_heads=n_heads, concat_heads=True)
    ...
    def forward(self,
        ...
        self.features_loss = torch.mean(torch.sqrt(
            torch.sum(torch.pow(node_feats-node_feats_recon, 2), axis=1)))

        # deconvolution loss
        log_lam = torch.log(torch.matmul(beta, basis) + 1e-6) + \
            alpha + self.gamma[slice_label]
