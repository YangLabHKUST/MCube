    ...
    if (method == "icp") or (method == "ICP"):
        ...
        for adata in adata_st_list:
            ...
            if data_type == "Visium":
                ...
                edge = ((n_neighbors > 1) & (
                    n_neighbors < 5)).astype(np.float32)
            else:
                loc_x = adata.obs.loc[:, ["array_row"]]
                ...
                pairwise_loc_distsq = np.sum(
                    (loc.reshape([1, -1, 2]) - loc.reshape([-1, 1, 2])) ** 2, axis=2)
                min_distsq = np.sort(
                    np.unique(pairwise_loc_distsq), axis=None)[1]
                n_neighbors = np.sum(pairwise_loc_distsq <
                                     (min_distsq * 3), axis=1) - 1
                edge = ((n_neighbors > 1) & (
                    n_neighbors < 7)).astype(np.float32)
            ...
            if test_all_angles == True:
                for angle in [0., np.pi * 1 / 3, np.pi * 2 / 3, np.pi, np.pi * 4 / 3, np.pi * 5 / 3]:
                    R = np.array([[np.cos(angle), np.sin(angle), 0],
                                  ...
                                  [0, 0, 1]]).T
                    T, distances, _ = icp(
                        transform(point_cloud_list[i+1], R), point_cloud_list[i], tolerance=tol)
                    if angle == 0:
                ...
                T, _, _ = icp(
                    point_cloud_list[i+1], point_cloud_list[i], tolerance=tol)
            trans_list.append(T)
        # Tranform
        for i in range(len(adata_st_list) - 1):
...
def preprocess(adata_st_list_input,  # list of spatial transcriptomics (ST) anndata objects
               ...
               save_path=None,  # save path for reference counts
               ):
    ...
    if save_path is not None:
        reference = pd.DataFrame(
            adata_basis.X, index=adata_basis.obs_names, columns=adata_basis.var_names)
        reference.to_csv(os.path.join(save_path, "reference_ST.csv"))
    ...
    if save_path is not None:
        ...
        adata_st.obs["library_size"].to_csv(
            os.path.join(save_path, "library_size_ST.csv"), index = True, header = True)
...
def select_hvgs(adata_ref, celltype_ref_col, num_per_group=200):
    sc.tl.rank_genes_groups(adata_ref, groupby=celltype_ref_col,
                            method="t-test", key_added="ttest", use_raw=False)
