...
def align_spots(
    adata_st_list_input,  # list of spatial transcriptomics datasets
    ...
    if plot:
        ...
        colors_list = [
            matplotlib.colors.rgb2hex(cmap(i)) for i in range(len(adata_st_list))
        ]
        ...
        for i in range(len(adata_st_list)):
            plt.scatter(
                adata_st_list[i].obsm[coor_key][:, 0],
                adata_st_list[i].obsm[coor_key][:, 1],
                c=colors_list[i],
                label="Slice %d spots" % i,
                s=5.0,
                alpha=0.5,
            )
        ax = plt.gca()
    ...
    if (method == "icp") or (method == "ICP"):
        ...
        for i in range(len(adata_st_list) - 1):
            if test_all_angles == True:
                for angle in [
                    ...
                    T, distances, _ = icp(
                        transform(point_cloud_list[i + 1], R),
                        point_cloud_list[i],
                        tolerance=tol,
                    )
                    if angle == 0:
        ...
        S1, S2 = generalized_procrustes_analysis(
            adata_st_list[0].obsm[coor_key], adata_st_list[1].obsm[coor_key], pis[0]
        )
    ...
    if plot:
        ...
        for i in range(len(adata_st_list)):
            plt.scatter(
                adata_st_list[i].obsm["spatial_aligned"][:, 0],
...
def calculate_impubasis(
    adata_st_input,  # st anndata object (should be one of the output from STitch3D.utils.preprocess)
