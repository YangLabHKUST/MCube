...
class Model():
    ...
    def __init__(self, adata_st, adata_basis,
                 ...
                 sparse=False  # use sparsemax or not in deconvolution layer
                 ):
        ...
        if scipy.sparse.issparse(adata_st.X):
            self.X = torch.from_numpy(
                adata_st.X.toarray()).float().to(self.device)
        else:
            self.X = torch.from_numpy(adata_st.X).float().to(self.device)
        self.A = torch.from_numpy(
            np.array(adata_st.obsm["graph"])).float().to(self.device)
        self.Y = torch.from_numpy(
            np.array(adata_st.obsm["count"])).float().to(self.device)
        self.lY = torch.from_numpy(np.array(
            adata_st.obs["library_size"].values.reshape(-1, 1))).float().to(self.device)
        self.slice = torch.from_numpy(
            np.array(adata_st.obs["slice"].values)).long().to(self.device)
        self.basis = torch.from_numpy(
            np.array(adata_basis.X)).float().to(self.device)
        ...
        for step in tqdm(range(self.training_steps)):
            ...
            if report_loss:
                if not step % step_interval:
                    print("Step: %s, Loss: %.4f, d_loss: %.4f, f_loss: %.4f" % (
                        step, loss.item(), self.net.decon_loss.item(), self.net.features_loss.item()))
    ...
    def eval(self, adata_st_list_raw, save=False, output_path="./results"):
        ...
        # save slice-specific platform effects
        platform_effects = self.gamma.detach().cpu().numpy()
        platform_effects = pd.DataFrame(data=platform_effects, index=[
                                        "slice%d" % i for i in range(self.n_slices)], columns=self.adata_st.var_names)
        if save == True:
            platform_effects.to_csv(os.path.join(
                output_path, "platform_effects_ST.csv"))

        # save slice- and spot-specific effects
        spot_effects = self.alpha.detach().cpu().numpy()
        spot_effects = pd.DataFrame(
            data=spot_effects, index=self.adata_st.obs.index)
        if save == True:
            spot_effects.to_csv(os.path.join(
                output_path, "spot_effects_ST.csv"))
        ...
        for i, adata_st_i in enumerate(adata_st_list_raw):
            ...
            if save == True:
                decon_res.to_csv(os.path.join(
                    output_path, "prop_slice%d.csv" % i))
                adata_st_i.write(os.path.join(
                    output_path, "res_adata_slice%d.h5ad" % i))

        # Save 3d coordinates
        if save == True:
            coor_3d = pd.DataFrame(
                data=self.adata_st.obsm['3D_coor'], index=self.adata_st.obs.index, columns=['x', 'y', 'z'])
    ...
    def cells_to_spatial(self, adata_ref_input,
        ...
        if celltype_ref is not None:
            if not isinstance(celltype_ref, list):
                ...
                adata_ref = adata_ref[[
                    (t in celltype_ref) for t in adata_ref.obs[celltype_ref_col].values.astype(str)], :]
        else:
        ...
        ref_counts = torch.from_numpy(ref_counts).to(
            torch.float32).to(self.device)  # N_cells x G

        celltype_onehot = np.zeros((adata_ref.shape[0], len(celltype_list)))
        for i in range(adata_ref.shape[0]):
            celltype_onehot[i, celltype_list.index(
                list(adata_ref.obs[celltype_ref_col].values)[i])] += 1.

        # Generate one-hot cell-type matrix for single cells
        celltype_onehot = torch.from_numpy(celltype_onehot).to(
            torch.float32).to(self.device)  # N_cells x C

        # Generate adjusted expression matrix for spatial spots
        Y_adjusted = (torch.matmul(self.beta, self.basis)
                      * self.lY).detach()  # N_spots x G

        beta = self.beta.detach()  # N_spots x C

        # N_cells x N_spots
        M = torch.zeros(adata_ref.shape[0], self.Y.shape[0])
        M = M.to(self.device)
        ...
        for step in tqdm(range(training_steps_M)):
            ...
            generated_spots = torch.matmul(torch.transpose(
                M_hat, 0, 1), ref_counts)  # N_spots x G
            loss_sim_spots = - \
                torch.mean(F.cosine_similarity(
                    Y_adjusted, generated_spots, dim=1))
            loss_sim_genes = - \
                torch.mean(F.cosine_similarity(
                    Y_adjusted, generated_spots, dim=0))

            generated_spots_prop = torch.matmul(torch.transpose(
                M_hat, 0, 1), celltype_onehot)  # N_spots x C
            generated_spots_prop = generated_spots_prop / \
                torch.sum(M_hat, axis=0).view(-1,
                                              1)  # Normalize generated proportions

            loss_prop = torch.mean(
                torch.sum((generated_spots_prop - beta) ** 2, dim=1))

            # regularizers
            target_num = adata_ref.shape[0] / self.Y.shape[0]
            reg_cell_num = torch.mean(
                (torch.sum(M_hat, axis=0) - target_num)**2)
            reg_M = -torch.mean(M_hat * torch.log(M_hat))
            loss_M = loss_prop + lam_sim * \
                (loss_sim_spots + loss_sim_genes) + \
                lam_num * reg_cell_num + lam_M * reg_M
        ...
        adata_ref.obsm['spatial_aligned'] = self.adata_st[np.argmax(
            self.M_hat, axis=1)].obsm['spatial_aligned']
        adata_ref.obsm['3D_coor'] = self.adata_st[np.argmax(
            self.M_hat, axis=1)].obsm['3D_coor']
        adata_ref.obs['slice'] = self.adata_st[np.argmax(
            self.M_hat, axis=1)].obs['slice'].values
